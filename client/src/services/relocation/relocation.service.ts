...
export class RelocationService implements OnDestroy {
    ...
    isPlayerRelocatable(player: Player): boolean {
        if (player == undefined) return false

        if (this.relocationTurns.length == 0) { return false }

        const currentRelocationTurn = this.relocationTurns[0]
        if (currentRelocationTurn.movementType !== 'relocation') return false

        if (currentRelocationTurn.playerFilter && currentRelocationTurn.playerFilter(player) === false) return false
    ...
    isPlayerMovable(player: Player): boolean {        
        ...
        if (currentRelocationTurn.movementType !== 'steps') return false
